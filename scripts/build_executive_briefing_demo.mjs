...
const demoDir = path.join(rootDir, "demo", "executive-briefing-machine");
const inputDir = path.join(demoDir, "inputs");
const outputDir = path.join(rootDir, "outputs", "executive-briefing-machine-demo");
...
function parseSectionedBullets(text) {
...
function createBriefingData(kpis, slack, github, leadershipNotes) {
  const latest = kpis.at(-1);
  const previous = kpis.at(-2);

  const metrics = [
    {
      label: "ARR",
      current: `$${latest.arrK}k`,
      delta: signed(delta(latest.arrK, previous.arrK), "k vs last month"),
      tone: "positive",
    },
    {
  ...
  const highlights = [
    ...slack["Highlights"].slice(0, 2),
    ...github["Shipping"].slice(0, 1),
  ];
  const risks = [
    ...slack["Risks"].slice(0, 2),
    ...github["Open Risks"].slice(0, 1),
  ];
  const asks = [
    ...slack["Asks"].slice(0, 2),
  ...
  return {
    ...
    narrative,
    leadershipNotes,
    kpis: kpis.map((row) => ({ ...row, monthLabel: monthLabel(row.month) })),
  };
}
...
async function buildWorkbook(data) {
  ...
  setColumnWidths(sources, [140, 120, 420]);
...
function buildDeck(data) {
  ...
  addCard(cover, {
    ...
    height: 230,
    title: "Period",
    body: `${data.periodLabel}\n\nGenerated from sample Slack, GitHub, KPI, and leadership-note inputs.`,
    accent: colors.green,
  });
  addCard(cover, {
    left: 460,
...
async function main() {
  ...
  const [slackText, githubText, leadershipText, csvText] = await Promise.all([
    fs.readFile(path.join(inputDir, "slack_updates.md"), "utf8"),
    fs.readFile(path.join(inputDir, "github_updates.md"), "utf8"),
    fs.readFile(path.join(inputDir, "leadership_notes.md"), "utf8"),
    fs.readFile(path.join(inputDir, "kpis.csv"), "utf8"),
  ]);

  const slack = parseSectionedBullets(slackText);
  const github = parseSectionedBullets(githubText);
  const leadershipNotes = leadershipText
    .split(/\r?\n/)
    .filter((line) => line.startsWith("- "))
    .map((line) => line.slice(2).trim());
  const kpis = parseCsv(csvText);

  const data = createBriefingData(kpis, slack, github, leadershipNotes);
